...
fn parse_number(input: &str) -> IResult<&str, u64> {
    map_res(recognize(digit1), |s: &str| s.parse::<u64>()).parse(input)
}

fn parse_list_of_numbers(input: &str) -> IResult<&str, Vec<u64>> {
    separated_list1(space1, parse_number).parse(input)
}
...
fn parse_line_of_numbers(input: &str) -> IResult<&str, Vec<u64>> {
    delimited(space0, parse_list_of_numbers, newline).parse(input)
}

fn parse_block_of_numbers(input: &str) -> IResult<&str, Vec<Vec<u64>>> {
    many1(parse_line_of_numbers).parse(input)
}
...
fn parse_list_of_operators(input: &str) -> IResult<&str, Vec<Op>> {
    separated_list1(space1, parse_op).parse(input)
}

fn consume_input(input: &str) -> IResult<&str, (Vec<Vec<u64>>, Vec<Op>)> {
    (parse_block_of_numbers, parse_list_of_operators).parse(input)
}
